...
def main():
    ...
    name = "franka_pnp_overfit1_fix3_50hz"
    ...
    if destination.exists():
        for key, expected_hash in manifest.items():
            ...
            if not path.is_file() or hashlib.sha256(path.read_bytes()).hexdigest() != expected_hash:
                raise ValueError(f"Existing dataset differs; preserve it and use a different HF_LEROBOT_HOME: {path}")
        print(f"Existing dataset verified: {destination}")
        return
    root.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix=".50hz-install-", dir=root) as temp:
