...
if [[ -n "${CAUSAL_RACE_DIR:-}" ]]; then
    claim_status=0
    "$python_bin" scripts/slurm/causal_race.py claim || claim_status=$?
    if [[ "$claim_status" == 10 ]]; then exit 0; fi
    if [[ "$claim_status" != 0 ]]; then exit "$claim_status"; fi
fi
args=(pi05_franka_pnp_overfit1_causal "--exp-name=$EXP_NAME" "--checkpoint-base-dir=$CHECKPOINT_BASE_DIR" "--fsdp-devices=$GPU_COUNT")
